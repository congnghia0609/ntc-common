 ...
 */
public abstract class AbstractThriftJacksonDeserializer <E extends TFieldIdEnum, T extends TBase<T, E>> extends JsonDeserializer<T> {
    private static final Logger log = LoggerFactory.getLogger(AbstractThriftJacksonDeserializer.class);
        ...
        while(iterator.hasNext()) {
            ...
            try {
                ...
                if(currentField.getValue().getNodeType() != JsonNodeType.NULL) {
                    final E field = getField(currentField.getKey());
                    ...
                    if(value != null) {
                        log.info("Field '{}' produced value '{}' of type '{}'.", currentField.getKey(), value, value.getClass().getName());
                        instance.setFieldValue(field, value);
                    } else {
                        log.info("Field '{}' contains a null value.  Skipping...", currentField.getKey());
                    }
                } else {
                    log.info("Field '{}' contains a null value.  Skipping...", currentField.getKey());
                }
    ...
    protected JavaType generateValueType(final T thriftInstance, final E field) throws NoSuchFieldException, SecurityException {
        ...
        if(declaredField.getType().equals(declaredField.getGenericType())) {
            log.info("Generating JavaType for type '{}'.", declaredField.getType());
            ...
            log.info("Generating JavaType for type '{}' with generics '{}'", declaredField.getType(), parameterizedTypes);
